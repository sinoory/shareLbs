...
AuthDbOpr.prototype.stationUpdate=function(msg,onlineUsers){
    mo.getOne({lineid:msg.lineid},function(err,line){
        ...
        if(!line) {
            console.log("stationUpdate line=null,ignore");
            return;
        }
...
AuthDbOpr.prototype.notifyBusUsers=function(onlineUsers,line,jsmsg){
    ...
    for(var i=0;i<line.watchers.length;i++){
        if(line.watchers[i] in onlineUsers){
            ...
            notifyeduses.push(line.watchers[i]);
        }
    }
    console.log("notifyeduses = ",notifyeduses);
}

AuthDbOpr.prototype.onLineChat=function(msg,onlineUsers){
    var notifyeduses=new Array();
    mo.getOne({'lineid':msg.lineid},function(err,line){
        //console.log("onLineChat line=",line);
        if(!line) return;
        for(var i=0;i<line.watchers.length;i++){
            if(msg.from==line.watchers[i]) continue;
            if(line.watchers[i] in onlineUsers){
                onlineUsers[line.watchers[i]].ws.send(JSON.stringify(msg));
                notifyeduses.push(line.watchers[i]);
            }
        }
        if(msg.from!=line.ownerid){//msg from passenger,so need send to line owner as not in line.watchers
            onlineUsers[line.ownerid].ws.send(JSON.stringify(msg));
            notifyeduses.push(line.ownerid);
        }
        console.log("onLineChat to = ",notifyeduses);
    });
}
...
AuthDbOpr.prototype.rmWatcher=function(watcherId,onlineUsers){
    if(!onlineUsers[watcherId]) return;
    var watchline=(onlineUsers[watcherId].watchline);
    if(watchline){
        mo.getOne({'lineid':watchline},function(err,line){
            if(line){
                for(var i=0;i<line.watchers.length;i++){
                    if(watcherId==line.watchers[i]){
                        line.watchers.splice(i,1);
                        line.save();
                    }
                }
            }
